...
def create_T3():
    ...
    global massive
    massive = [[0] * (len(column_names_T1) + 2) for _ in range(len(row_T1) + 2)]
    ...
    massive[len(row_T1) + 1][0] = "Добавить столбец"
    massive[len(row_T1) + 1][1] = tk.IntVar()  # Один чекбокс
    massive[len(row_T1) + 1][2] = tk.StringVar()  # Для выпадающего списка
